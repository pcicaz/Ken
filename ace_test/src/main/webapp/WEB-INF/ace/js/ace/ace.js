		...
		var demo_ajax_options = {
			 ...
			 'content_url': function(hash) {
				...
				//please refer to documentation for more info
				 ...
				 //for passing parameter 05/25/15 by wangcheng
				 var combo = hash.split("?");
				 hash = combo[0];
				 //for passing parameter 05/25/15 by wangcheng
				 ...
				 if (path.match(/(\/index)?/)) {
					 var p = path.replace('/index', '/html/content/' + hash.replace(/^page\//, '') + '.html');
					 if (combo.length > 1) { //for passing parameter 05/25/15 by wangcheng
						 p += '?' + combo[1];
					 }
					 return p;
				 }
